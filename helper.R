...
# Edited: Anne Hackman, Spring 2024                                                             #
#################################################################################################
...
# Edited: Kody DeGolier,  Summer 2022                                                                                #
# Update: years 2022 and 2023 added to section (a)                                                                   #
# Edited: Anne Hackman, Spring 2024                                                                                  #
######################################################################################################################
...
make_impact_plot <- function(measure, mean_pm2.5_chosen, dat, plot_site){
  ...
  if(mean_pm2.5_chosen == "mean_pm2.5_2022_from2.5") {current_year = "2022"}
  if(mean_pm2.5_chosen == "mean_pm2.5_2023_from2.5") {current_year = "2023"}
...
impact_plot_text <- function(measure, var1, dat){
  ...
  if(measure == "Distance to Nearest Interstate"){
    ...
    text <- paste0("For every mile you move away from the interstate, the average PM2.5 measure decreases by ", beta, " micro g/(cubic m). ", 
  ...
  if(measure == "Distance to Nearest Power Plant"){
    ...
    r2 <- round(summary(fit)$r.squared,4)*100
    beta <- -1*round(coefficients(fit)[2],4)
    text <- paste0("For every mile you move away from a power plant, the average PM2.5 measure decreases by ", beta, " micro g/(cubic m). ", 
  ...
  # Median Income
  # Rounded to six decimal places because the beta for some years is quite small
  if(measure == "Median Income"){
    dat2 = dat[!is.na(dat$med_income),]
    fit <- lm(dat2[[var1]] ~ med_income, data = dat2)
    r2 <- round(summary(fit)$r.squared,4)*100
    beta <- round(coefficients(fit)[2],6)*1000
    text <- paste0("For every $1,000 increase in median income, the average PM2.5 increases by ", beta, " micro g/(cubic m). ", 
...
# Credit: Rachel Zilinskas                                                                                #
# Update: geocode function queries US Census service due to its lack of usage limitations                 #
# Edited: Anne Hackman, Spring 2024                                                                       #
###########################################################################################################
...
# Edited: Kody DeGolier,  Summer 2022                                                                         #
# Edited: Anne Hackman, March 2024                                                                            #
###############################################################################################################
...
seasonal_plot <- function(site1, site2, year){
  ...
  p <- seasonal_data_order %>% 
    ...
    # Adding rectangles so that the background of the plot is colored correctly
    geom_rect(fill = "#CCE6CC", alpha = 0.1, xmin = 0, xmax = 15, ymin = 0, ymax = 12) +
    geom_rect(fill = "#FFFFCC", alpha = 0.1, xmin = 0, xmax = 15, ymin = 12, ymax = 35.5) +
...
# for the years 2015 - 2023. Users also have the option to highlight a specific site to be seen on the plots   #
...
make_weather_plot <- function(site, weather.var, data, year, time) {
  ...
  # clean name of weather variable for x-axis
  xax.paste = str_replace_all(str_replace_all(str_replace_all(weather.var,"[.]"," "),"degrees  F","(F)")," inches","(in.)")
  
  if(time=="Weekly"){
    xax <- list(
      title = paste0("Average Weekly ", xax.paste), zeroline = FALSE,
      showline = FALSE
      )
  }else{
    xax <- list(
      title = paste0("Daily ", xax.paste),zeroline = FALSE,
      showline = FALSE
    )
  }
  yax <- list(
  ...
  # defines top boundary of red rect object in code below
  # will need to be edited if any daily max PM2.5 value in updated data exceeds 110
  ymax.red <- ifelse(max(dat$PM2.5) > 72, 110, 75)
...
#The code below causes the table used in the weather plot to be shaded based off the PM2.5 color scale rather than plain in color #
  ...
  fig<-fig%>%layout(xaxis = xax, yaxis = yax,shapes = list(
    ...
    list(type = "rect",
         ...
         line = list(color = "red"), opacity = 0.2,
         x0 = min(dat$weather), x1 = max(dat$weather), xref = "xax",
         y0 = 55.5, y1 = ymax.red, yref = "yax")))
  if(time=="Weekly"){
    fig<-fig%>%add_trace( x = ~weather, y = ~PM2.5,type = 'scatter', mode = 'markers', text = ~paste( "PM2.5 for",site.name,"on",time.var,Time,":", PM2.5),hoverinfo = 'text', marker = list(color='black'),showlegend = F)%>%add_lines(x = ~weather, y = ~fit,color=~site.name,showlegend=T) %>%
    config(displayModeBar = FALSE) %>%
    layout(showlegend = FALSE, dragmode = FALSE)
  }else{
    fig<-fig%>%add_trace( x = ~weather, y = ~PM2.5,type = 'scatter', mode = 'markers', text = ~paste( "PM2.5 for",site.name,"on",time.var,Time,":", PM2.5),hoverinfo = 'text', marker = list(color='black'),showlegend = F)%>%add_lines(x = ~weather, y = ~fit,color=~site.name,showlegend=T) %>%
...
pm2.5_about_text <- function(selected_level) {
  ...
  if(selected_level == "Moderate"){
    ...
    text <- ("EPA Definition of 'Moderate':
  ...
  if(selected_level == "Unhealthy for Sensitive Groups"){
    ...
    text <- ("EPA Definition of 'Unhealthy for Sensitive Groups':
  ...
  if(selected_level == "Unhealthy"){
    ...
    text <- ("EPA Definition of 'Unhealthy':
